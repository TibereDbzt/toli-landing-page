/* Landing sections A: NavBar, Hero, Problem, Product feature rows. */

const { Button: BtnL, Tag: TagL } = window.TM;
const E = window.Eyebrow;
...
function Proof({ children }) {
  return (
    <span style={{
      ...
      <span style={{ display: "inline-flex", color: "var(--accent)" }}><window.Ic.check size={15} /></span>
      {children}
    </span>
  );
}
    ...
    <section id="top" style={{ paddingTop: 76, paddingBottom: 40, position: "relative", overflow: "hidden" }}>
      <Container w={980} style={{ textAlign: "center" }}>
        ...
        <div className="reveal" style={{ display: "flex", gap: 13, justifyContent: "center", marginTop: 34, flexWrap: "wrap" }}>
          <a href="#demo" style={{ textDecoration: "none" }}><BtnL size="lg" iconRight={<window.Ic.arrowRight size={17} />}>Book a demo</BtnL></a>
      ...
      <Container w={1180} style={{ marginTop: 54 }}>
        <div className="reveal hero-frame">
          <window.Win chrome="app.toli.run / utmb · overview">
            <window.EventMock />
          </window.Win>
        </div>
      </Container>
  ...
  return (
    <div style={{
      ...
      <div style={{ height: 30, display: "flex", alignItems: "center", gap: 7, padding: "0 12px", borderBottom: "1px solid var(--hairline)", background: "var(--surface-well)" }}>
        <window.Ic.list size={13} style={{ color: "var(--text-faint)" }} />
...
/* ============================== PRODUCT FEATURES ============================== */
function FeatureRow({ idx, n, eyebrow, title, body, proofs, mock, chrome, flip }) {
  const text = (
  ...
  const visual = (
    <div style={{ flex: "1 1 540px", minWidth: 0 }}>
      <window.Win chrome={chrome}><window.Fit width={920}>{mock}</window.Fit></window.Win>
    </div>
  );
  return (
...
Object.assign(window, { Container, BrandMark, NavBar, Hero, Problem, Product });
