/* Landing sections B: For athletes, Day-with/without, Why Toli, Trusted, FAQ, Final CTA, Footer. */

const { Button: BtnB } = window.TM;
const Cont = window.Container;
const Eb = window.Eyebrow;
    ...
    <section id="athletes" style={{ background: "var(--surface-raised)", borderTop: "1px solid var(--hairline)", borderBottom: "1px solid var(--hairline)", padding: "100px 0" }}>
      <Cont w={1100} style={{ display: "flex", gap: 60, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: "26px 0 0", display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              ...
              <li key={t} style={{ display: "flex", gap: 13 }}>
                <span style={{ flex: "none", marginTop: 2, color: "var(--accent)" }}><window.Ic.checkCircle size={20} /></span>
                <span>
        ...
        <div className="reveal" style={{ flex: "1 1 420px", display: "flex", gap: 22, justifyContent: "center" }}>
          <window.AthleteRegMock />
          <div style={{ marginTop: 44 }}><window.AthleteInboxMock /></div>
        </div>
      </Cont>
    </section>
  );
    ...
    <section style={{ padding: "104px 0" }}>
      <Cont w={1100}>
        ...
        <div className="reveal" style={{ display: "flex", gap: 22, flexWrap: "wrap", marginTop: 40 }}>
          <Col tone="bad" eyebrow="Without Toli" title="Six tabs and a bad feeling" items={without} glyph={<window.Ic.x size={17} />} gc="var(--danger)" />
          <Col tone="good" eyebrow="With Toli" title="One screen, one truth" items={wth} glyph={<window.Ic.check size={17} />} gc="var(--accent-on)" />
        </div>
      </Cont>
    </section>
  );
  ...
  return (
    <div style={{ borderBottom: "1px solid var(--hairline)" }}>
      <button onClick={onToggle} style={{
        ...
        <span style={{ flex: "none", color: "var(--text-muted)", transform: open ? "rotate(45deg)" : "none", transition: "transform .2s" }}><window.Ic.plus size={22} /></span>
      </button>
        ...
        <div className="reveal" style={{ display: "flex", gap: 56, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <form onSubmit={(e) => e.preventDefault()} style={{ flex: "1 1 380px", background: "var(--surface-well)", border: "1px solid var(--hairline)", borderRadius: 20, padding: 28 }}>
            ...
            <div style={{ marginTop: 20 }}>
              <BtnB size="lg" type="submit" style={{ width: "100%" }} iconRight={<window.Ic.arrowRight size={17} />}>Book a demo</BtnB>
            </div>
...
Object.assign(window, { Athletes, DayCompare, Why, Trusted, FAQ, FinalCTA, Footer });
