/* Manager mockups B: Attendance & logistics, Targeted messaging (read receipts),
   Performance insights (UTMB sync). */

const { Avatar: AvatarB, Tag: TagB, StatusDot: StatusDotB, StatCard: StatCardB, Metric: MetricB, Button: ButtonB } = window.TM;
...
function field(label, value, glyph) {
  ...
  return (
    <div>
      <window.Eyebrow style={{ fontSize: 10, marginBottom: 6 }}>{label}</window.Eyebrow>
      <div style={{
...
function AttendanceMock() {
  const cal = <window.Ic.chevronDown size={14} />;
  const rows = [
  ...
  return (
    <div style={{ background: "var(--surface-app)", padding: "26px 30px", minHeight: 520 }}>
      <window.Eyebrow style={{ color: "var(--text-muted)" }}>UTMB · Attendance</window.Eyebrow>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
        ...
        <span style={{ fontFamily: "var(--font-ui)", fontSize: 12.5, color: "var(--text-faint)", display: "inline-flex", alignItems: "center", gap: 6 }}>
          <window.Ic.alert size={15} /> 1 athlete missing arrival
        </span>
      </div>
        ...
        {rows.map((r) => (
          <div key={r.n} style={{ background: "var(--surface-card)", border: "1px solid var(--hairline)", borderRadius: 14, padding: 16, boxShadow: "var(--shadow-card)", display: "grid", gridTemplateColumns: "minmax(180px,1fr) repeat(4, minmax(120px,1fr))", gap: 14, alignItems: "center" }}>
            <span style={{ display: "flex", alignItems: "center", gap: 11 }}>
              <AvatarB name={r.n} initials={r.i} size="sm" />
              <span>
...
function MessagingMock() {
  ...
  return (
    <div style={{ background: "var(--surface-app)", padding: "26px 30px", minHeight: 520, display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 22 }}>
      <div>
        <window.Eyebrow style={{ color: "var(--text-muted)" }}>New message</window.Eyebrow>
        ...
        <window.Eyebrow style={{ fontSize: 10, marginBottom: 7 }}>Recipients</window.Eyebrow>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginBottom: 16 }}>
          {[["Whole team", false], ["Everyone at UTMB", false], ["Racing CCC", true], ["One athlete", false]].map(([l, on]) => (
            ...
            }}>{on ? <window.Ic.check size={14} /> : null}{l}</span>
          ))}
        </div>
        ...
        <div style={{ background: "var(--surface-card)", border: "1px solid var(--hairline)", borderRadius: 14, padding: 16, boxShadow: "var(--shadow-card)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            ...
            <span style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "5px 10px", borderRadius: 999, background: "var(--danger-soft)", color: "var(--danger)", fontFamily: "var(--font-ui)", fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" }}>
              <window.Ic.alert size={13} /> Urgent
            </span>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 14 }}>
            <ButtonB size="sm" icon={<window.Ic.send size={15} />}>Send</ButtonB>
          </div>
        </div>
      </div>

      <div>
        <window.Eyebrow style={{ fontSize: 10, marginBottom: 10 }}>Read receipts · 3 of 5</window.Eyebrow>
...
function PerfMock() {
  ...
  return (
    <div style={{ display: "flex", background: "var(--surface-app)", minHeight: 540 }}>
      <window.Rail active="chart" />
      <div style={{ flex: 1, padding: "26px 30px" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
          <div>
            <window.Eyebrow style={{ color: "var(--text-muted)" }}>Performance · 2025 season</window.Eyebrow>
          ...
          <ButtonB variant="secondary" size="sm" icon={<window.Ic.download size={15} />}>Export CSV</ButtonB>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginTop: 22 }}>
          <StatCardB label="Victories" value="14" sub="across the roster" />
          <StatCardB label="Podiums" value="38" sub="top-3 finishes" />
          <StatCardB label="Avg UTMB index" value="901" unit="W" sub="women · 884 men" />
          <StatCardB label="Elite tier" value="6" sub="of 18 · World Class" />
        </div>

        <div style={{ marginTop: 28, display: "flex", alignItems: "center", gap: 12 }}>
          <AvatarB name="Émile Auclair" initials="ÉA" size="sm" />
          ...
          <span style={{ marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 7, fontFamily: "var(--font-ui)", fontSize: 12, fontWeight: 600, color: "var(--text-faint)" }}>
            <window.Ic.refresh size={14} /> Synced from utmb.world
          </span>
        </div>
        ...
        <div style={{ marginTop: 14, background: "var(--surface-card)", border: "1px solid var(--hairline)", borderRadius: 16, overflow: "hidden", boxShadow: "var(--shadow-card)" }}>
          <div style={{ display: "grid", gridTemplateColumns: cols, gap: 14, padding: "13px 20px", borderBottom: "1px solid var(--hairline)" }}>
            {["Race", "Distance", "D+", "Rank", "Finish", "UTMB index"].map((h) => <window.Eyebrow key={h} style={{ fontSize: 10 }}>{h}</window.Eyebrow>)}
          </div>
          {results.map((r, i) => {
            ...
            return (
              <div key={r.race} style={{ display: "grid", gridTemplateColumns: cols, gap: 14, alignItems: "center", padding: "14px 20px", borderTop: i ? "1px solid var(--hairline)" : "none" }}>
                ...
                {r.idx ? <MetricB value={r.idx} size="sm" /> : <span style={{ color: "var(--text-faint)" }}>—</span>}
              </div>
            );
          })}
        </div>
...
Object.assign(window, { AttendanceMock, MessagingMock, PerfMock });
